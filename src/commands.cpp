...
bool Answer::RunCommand(const std::string &loopsuffix)
{
  ...
  do
  {
    time_t secsafter = time(NULL);
    if (expire < secsafter)
    {
      expire = secsafter + 60;
      TPState::Instance().GetManager()->RegisterSIP();
    }
...
bool Record::RunCommand(const std::string &loopsuffix)
{
  ...
  if (iterationsuffix)
  {
    ...
    if (ext == P_MAX_INDEX)
      ...
      filename =
          audiofilename.Left(ext) + loopsuffix + audiofilename.Mid(ext);
  }
  else
...
bool Wait::RunCommand(const std::string &loopsuffix)
{
  ...
  {
    // silence detection
    if (silence && TPState::Instance().IsSilent(
    ...
    // activity detection
    else if (activity && TPState::Instance().IsActive(
    ...
    if (closed && (TPState::Instance().GetState() == TPState::TERMINATED || TPState::Instance().GetState() == TPState::CLOSED))
    {
